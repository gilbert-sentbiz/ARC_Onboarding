...
import { useRuleStore, getRuleSet } from '@/src/entities/rule/model/ruleStore'
import type {
  EntityCode,
  ServiceCode,
  SectorCode,
  ServiceClassificationRule,
  DocTemplateRule,
  QuestionRule,
  SegmentQuestionConfig,
  QuestionInputType,
  EntityClassificationRule,
  EntityClassificationCondition,
  FirstIntakeQuestion,
  DocLibraryItem,
  DocSegmentConfig,
...
type Selection =
  { type: 'entity'; code: EntityCode } | { type: 'service'; code: ServiceCode } | { type: 'intake' }
...
function ScreenDivider({ n }: { n: number }) {
  return (
    <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 border-y border-blue-100">
      ...
      <span className="text-[11px] font-bold text-sb-brand whitespace-nowrap tracking-wide">
        고객 화면 {n}
      </span>
...
function AddCommonQuestionForm({
  onAdd,
  onCancel,
}: {
  ...
  const allSegKeys: { key: string; label: string }[] = [
    ...
    ...(Object.keys(rs.serviceLabels) as ServiceCode[]).map((s) => ({
      key: `service:${s}`,
      label: rs.serviceLabels[s] ?? s,
    })),
  ]
  ...
  return (
    <div className="border-t border-sb-n100 px-4 py-3 bg-sb-n50 flex flex-col gap-3">
      ...
      <div className="flex items-end gap-2">
        ...
        <div className="w-24">
          ...
          <select
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, inputType: e.target.value as QuestionInputType }))
            }
      ...
      {qHasOptionField && (
        <div className="flex flex-col gap-1.5">
          <p className="text-[11px] text-sb-n400">
            옵션 <span className="text-sb-n300">(value / label)</span>
          </p>
          {optionRows.map((opt, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <input
                ...
                onChange={(e) =>
                  setOptionRows((r) =>
                    r.map((o, j) => (j === i ? { ...o, value: e.target.value } : o))
                  )
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setOptionRows((r) =>
                    r.map((o, j) => (j === i ? { ...o, label: e.target.value } : o))
                  )
                }
                className="flex-1 text-[12px] border border-sb-n200 rounded-[6px] px-2 py-1 bg-white focus:outline-none focus:border-sb-brand"
              />
              <button
                onClick={() => setOptionRows((r) => r.filter((_, j) => j !== i))}
                className="text-sb-n300 hover:text-sb-negative"
              >
                <Trash size={12} />
              </button>
            </div>
          ...
          <button
            onClick={() => setOptionRows((r) => [...r, { value: '', label: '' }])}
      ...
      <div>
        <p className="text-[11px] text-sb-n400 mb-1.5">
          적용 세그먼트 <span className="text-sb-n300">(다중 선택)</span>
        </p>
        <div className="flex flex-wrap gap-x-4 gap-y-1.5">
          {allSegKeys.map(({ key, label }) => (
            <label key={key} className="flex items-center gap-1.5 cursor-pointer">
              <input
                type="checkbox"
                checked={selectedSegments.includes(key)}
                onChange={() => toggleSeg(key)}
                className="rounded border-sb-n300 text-sb-brand focus:ring-sb-brand"
              />
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={onCancel}
          className="text-[12px] text-sb-n500 hover:text-sb-n800 px-2 py-1.5"
        >
          취소
        </button>
      </div>
...
function CommonQuestionRow({
  ...
  return (
    <>
      ...
      >
        <div className="flex items-start gap-3 px-4 py-3">
          ...
          <span className="flex items-center gap-1 flex-shrink-0 mt-0.5">
            ...
            {qHasChildren && (
              <button
                ...
                className="text-sb-n400 hover:text-sb-brand"
              >
          ...
          <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
            ...
            <span
              className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${q.isRequired ? 'bg-red-50 text-sb-negative' : 'bg-sb-n50 text-sb-n400'}`}
            >
              {q.isRequired ? '필수' : '선택'}
            </span>
            <span className="text-[10px] font-mono text-sb-n400 px-1.5 py-0.5 border border-sb-n100 rounded">
              {q.inputType}
            </span>
            {onDelete && (
              <button
      ...
      {expanded &&
        q.children?.map((child) => <QuestionRowFixed key={child.id} q={child} depth={1} />)}
    </>
  )
}

function QuestionRowFixed({
  q,
  depth = 0,
  allConfigs = [],
  currentSegKey = '',
          ...
          {qHasOptions && depth === 0 && (
            <div className="flex flex-wrap gap-1 mt-1.5">
              {q.options!.map((opt) => (
                <span
                  key={opt.value}
                  className="text-[11px] px-2 py-0.5 rounded-full border bg-sb-n50 border-sb-n200 text-sb-n600"
                >
                  {opt.label}
                </span>
              ))}
...
function AddOwnQuestionForm({
  ...
  function submit() {
    ...
    setForm({
      ...
      parentId: '',
      parentValue: '',
      svcCondition: defaultSvcCondition ?? '',
    })
    setOptionRows([])
  }
  ...
  return (
    <div className="pt-3 border-t border-sb-n100 mt-1 flex flex-col gap-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setForm((p) => ({
                ...p,
                svcCondition: e.target.value as typeof p.svcCondition,
                parentId: '',
                parentValue: '',
              }))
            }
      ...
      <div className="flex items-end gap-2">
        ...
        {onCancel && (
          <button onClick={onCancel} className="text-[12px] text-sb-n500 hover:text-sb-n800 mb-px">
            취소
          </button>
        )}
      </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1">
            ...
            <select
              value={form.parentId}
              onChange={(e) =>
                setForm((p) => ({ ...p, parentId: e.target.value, parentValue: '' }))
              }
            ...
            >
              ...
              {allParents
                .filter((pq) => pq.options?.length)
                .map((pq) => (
                  <option key={pq.id} value={pq.id}>
                    [{pq.inputType}] {pq.label}
                  </option>
                ))}
              {allParents
                .filter((pq) => !pq.options?.length)
...
function FirstQuestionsEditor() {
  ...
  return (
    <div className="flex flex-col gap-5">
      <div>
        <p className="text-[12px] text-sb-n500 mb-3">
          모든 고객이 세그먼트 분류 전에 작성하는 인테이크 폼입니다. 여기서 받은 값으로 세그먼트가
          결정됩니다.
        </p>
        <div className="bg-white rounded-[12px] border border-sb-n100 overflow-hidden">
          {questions.map((q, idx) => {
            ...
            return (
              <div
                ...
                onDrop={() => {
                  if (dragSrc !== null && dragSrc !== idx) reorderFirst(dragSrc, idx)
                  setDragSrc(null)
                  setDragOver(null)
                }}
          ...
          <div className="px-4 py-3 bg-sb-n50 flex flex-col gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={addType}
                onChange={(e) => setAddType(e.target.value as QuestionInputType)}
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                <input
                  ...
                  className="rounded border-sb-n300 text-sb-brand"
                />
                <span className="text-[12px] text-sb-n600">필수</span>
              </label>
              <button
...
function QuestionsEditor({ selected }: { selected: Selection }) {
  ...
  const config: SegmentQuestionConfig = rs.segmentQuestionConfigs.find(
    (c) => c.key === configKey
  ...
  const ownFixedQuestions = rs.questionPool.filter((q) => {
    if (selected.type === 'entity')
      return (
        q.classification === 'entity-own' &&
        (q.scope === selected.code || q.scopeEntity === selected.code)
      )
    if (selected.type === 'service')
      return (
        q.classification === 'service-own' &&
        (q.scope === selected.code || q.scopeService === selected.code)
      )
    return false
  })
  ...
  function saveConfig(patch: Partial<SegmentQuestionConfig>) {
    ...
    updateRuleSet({
      ...currentRuleSet,
      version: nextVersion(currentRuleSet.version),
      segmentQuestionConfigs: updated,
      questionPool: fullRs.questionPool,
    })
  }
  ...
  function deleteCommonQuestion(q: QuestionRule) {
    const fullRs = getRuleSet()
    const mappedCount = fullRs.segmentQuestionConfigs.filter((c) =>
      c.enabledCommonQuestionIds.includes(q.id)
    ).length
    const msg =
      mappedCount > 1
    ...
    if (mappedCount > 1) {
      const updatedConfigs = fullRs.segmentQuestionConfigs.map((c) =>
        c.key === configKey
          ? {
              ...c,
              enabledCommonQuestionIds: c.enabledCommonQuestionIds.filter((id) => id !== q.id),
            }
          : c
      )
      updateRuleSet({
        ...
        version: nextVersion(currentRuleSet.version),
        questionPool: fullRs.questionPool,
        segmentQuestionConfigs: updatedConfigs,
      })
    } else {
  ...
  function deletePoolOwnQuestion(q: QuestionRule) {
    if (
      !window.confirm(
        `"${q.label}" 질문을 삭제합니다. 하위 질문도 함께 삭제됩니다.\n계속하시겠습니까?`
      )
    )
      return
    const fullRs = getRuleSet()
  ...
  function reorderCommon(from: number, to: number) {
    ...
    updateRuleSet({
      ...
      version: nextVersion(currentRuleSet.version),
      questionPool: pool,
      segmentQuestionConfigs: fullRs.segmentQuestionConfigs,
    })
  }
  ...
  function addCommonQuestion(q: QuestionRule, segmentKeys: string[]) {
    ...
    const newConfigs: SegmentQuestionConfig[] = [
      ...
      ...segmentKeys
        ...
        .map((k) => ({
          key: k,
          enabledCommonQuestionIds: [q.id],
          ownQuestions: [] as QuestionRule[],
        })),
    ]
    updateRuleSet({
      ...currentRuleSet,
  ...
  const segmentCode =
    selected.type === 'entity' ? selected.code : selected.type === 'service' ? selected.code : ''
    ...
    <div className="flex flex-col gap-5">
      ...
      {isCollection && (
        <div className="flex items-start gap-3 px-4 py-3 rounded-[10px] bg-blue-50 border border-blue-100">
          <span className="text-sb-brand flex-shrink-0 mt-0.5 text-[15px] font-bold leading-none">
            ℹ
          </span>
          <p className="text-[12px] text-blue-800">
            이 고객은 <strong>FI 질문 전체 + 아래 통화 고유 질문</strong>을 받습니다 (수금=FI). FI
            공통 질문은 <strong>FI 세그먼트</strong>에서 관리하고, 여기서는 통화 고유 질문만
            추가합니다.
          </p>
        </div>
      )}
            ...
            {commonQuestions.map((q, cidx) => (
              <CommonQuestionRow
                ...
                dragProps={{
                  ...
                  onDragEnd: () => {
                    setDragCommonSrc(null)
                    setDragCommonOver(null)
                  },
                  onDragOver: (e) => {
                    e.preventDefault()
                    setDragCommonOver(cidx)
                  },
                  onDrop: () => {
                    if (dragCommonSrc !== null && dragCommonSrc !== cidx)
                      reorderCommon(dragCommonSrc, cidx)
                    setDragCommonSrc(null)
                    setDragCommonOver(null)
                  },
        ...
        <div>
          ...
          {svcView !== 'all' && (
            <p className="text-[11px] text-sb-n400 mt-1.5">
              {svcView === 'payout' ? '송금' : '수금'} 서비스 고객에게만 표시되는 질문. 서비스 조건
              없는 질문은 항상 표시됩니다.
            </p>
          )}
        </div>
      ...
      <div>
        ...
        <div className="bg-white rounded-[10px] border border-sb-n100 overflow-hidden">
          {ownFixedQuestions.filter(matchesSvcView).length === 0 &&
            config.ownQuestions.filter(matchesSvcView).length === 0 && (
            ...
            <Fragment key={q.id}>
              <QuestionRowFixed
                ...
                dragProps={{
                  ...
                  onDragEnd: () => {
                    setDragFixedSrc(null)
                    setDragFixedOver(null)
                  },
                  onDragOver: (e) => {
                    e.preventDefault()
                    setDragFixedOver(idx)
                  },
                  onDrop: () => {
                    if (dragFixedSrc !== null && dragFixedSrc !== idx)
                      reorderFixed(dragFixedSrc, idx)
                    setDragFixedSrc(null)
                    setDragFixedOver(null)
                  },
            ...
            return config.ownQuestions.map((q, i) => {
              ...
              return (
                <div
                  ...
                  onDragEnd={() => {
                    setDragConfigSrc(null)
                    setDragConfigOver(null)
                  }}
                  onDragOver={(e) => {
                    e.preventDefault()
                    setDragConfigOver(visIdx)
                  }}
                  onDrop={() => {
                    if (dragConfigSrc !== null && dragConfigSrc !== visIdx)
                      reorderConfigOwn(dragConfigSrc, visIdx)
                    setDragConfigSrc(null)
                    setDragConfigOver(null)
                  }}
  ...
  return (
    <div className="flex flex-col divide-y divide-sb-n100">
      {docs.map((doc, i) => (
        <div
          key={doc.type}
          className="grid grid-cols-[1fr_110px_80px_32px] gap-2 items-center py-2"
        >
          <input
          ...
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input
              ...
              onChange={(e) =>
                onUpdate(i, { isRequired: e.target.checked, isConditional: !e.target.checked })
              }
              className="rounded border-sb-n300 text-sb-brand focus:ring-sb-brand"
            />
            <span className="text-[12px] text-sb-n600">필수</span>
...
function DocLibraryRow({
        ...
        <div className="flex flex-col gap-2 bg-sb-blue-50 rounded-[8px] border border-sb-blue-200 px-3 py-2.5">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="text-[12px] font-medium text-white bg-sb-brand px-3 py-1.5 rounded-[6px]"
            >
              저장
            </button>
            <button
  ...
  return (
    <div
      className={`flex items-start gap-3 px-4 py-3 border-b border-sb-n100 last:border-0 ${!enabled ? 'opacity-40' : ''}`}
    >
      <span className="flex-shrink-0 mt-0.5">
        <ToggleSwitch checked={enabled} onChange={onToggle} />
      </span>
      ...
      <div className="flex items-center gap-1 flex-shrink-0 mt-0.5">
        {override?.displayName && (
          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-amber-50 border border-amber-200 text-amber-700">
            오버라이드
          </span>
        )}
        <span
          className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${doc.isRequired && !doc.isConditional ? 'bg-red-50 text-sb-negative' : 'bg-sb-n50 text-sb-n400'}`}
        >
...
function DocumentsEditor({ selected }: { selected: Selection }) {
  ...
  function saveSegConfig(patch: Partial<DocSegmentConfig>) {
    ...
    updateRuleSet({
      ...currentRuleSet,
      version: nextVersion(currentRuleSet.version),
      segmentDocConfigs: updated,
      docLibrary: latest.docLibrary,
    })
  }
  ...
  function toggleCommon(type: string) {
    ...
    saveSegConfig({
      enabledCommonDocTypes: enabled.includes(type)
  ...
  function updateSectorDocs(
    sector: SectorCode,
    updater: (docs: DocTemplateRule[]) => DocTemplateRule[]
  ) {
    const svcCode = selCode as ServiceCode
    const updated = rs.documentRules.map((rule) => {
      if (rule.match.service !== svcCode || rule.match.sector !== sector || rule.match.entity)
        return rule
          ...
          <div className="bg-white rounded-[10px] border border-sb-n100 overflow-hidden">
            {ownFixedDocs.map((doc) => {
              ...
              return (
                <div
                  key={doc.type}
                  className={`flex items-start gap-3 px-4 py-3 border-b border-sb-n100 last:border-0 ${!on ? 'opacity-40' : ''}`}
                >
                  <span className="flex-shrink-0 mt-0.5">
                  ...
                  <span
                    className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full flex-shrink-0 mt-0.5 ${doc.isRequired && !doc.isConditional ? 'bg-red-50 text-sb-negative' : 'bg-sb-n50 text-sb-n400'}`}
                  >
                    {doc.isRequired && !doc.isConditional ? '필수' : '조건부'}
                  </span>
                </div>
      ...
      {hasKRWSectors && (
        <div className="flex flex-col gap-3">
          ...
          {SECTOR_ORDER.map((sector) => {
            const sectorRule = rs.documentRules.find(
              (r) => r.match.service === selCode && r.match.sector === sector
            )
            ...
            return (
              <div key={sector} className="bg-white rounded-[10px] border border-sb-n100 p-4">
                ...
                <DocList
                  docs={sectorDocs}
                  onUpdate={(i, patch) =>
                    updateSectorDocs(sector, (d) =>
                      d.map((doc, idx) => (idx === i ? { ...doc, ...patch } : doc))
                    )
                  }
                ...
                <AddDocForm
                  onAdd={(doc) => {
                    if (sectorRule) {
                      ...
                      const newRule = {
                        match: { service: selCode as ServiceCode, sector },
                        docs: [doc],
                      }
                      updateRuleSet({
                        ...currentRuleSet,
                        version: nextVersion(currentRuleSet.version),
                        documentRules: [...rs.documentRules, newRule],
                      })
                    }
                  }}
                />
      ...
      <div>
        ...
        <div className="bg-white rounded-[10px] border border-sb-n100 overflow-hidden">
          ...
          {config.ownDocs.map((doc, i) => (
            <div
              key={doc.type + i}
              className="flex items-start gap-3 px-4 py-3 border-b border-sb-n100 last:border-0"
            >
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-sb-n800">{doc.displayName}</p>
                <p className="text-[11px] font-mono text-sb-n400 mt-0.5">{doc.type}</p>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <button
                  onClick={() =>
                    saveSegConfig({ ownDocs: config.ownDocs.filter((_, j) => j !== i) })
                  }
                  className="flex items-center justify-center w-7 h-7 rounded-[6px] text-sb-n400 hover:text-sb-negative hover:bg-red-50 transition-colors"
                >
                  <Trash size={13} />
            ...
            <div className="border-t border-sb-n100 px-4 py-3 bg-sb-n50 flex flex-col gap-2">
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setAddDocForm((f) => f && { ...f, type: e.target.value })}
                  placeholder="type (예: MY_DOC)"
                  className="w-36 text-[12px] font-mono border border-sb-n200 rounded-[6px] px-2 py-1.5 bg-white focus:outline-none focus:border-sb-brand"
                />
                <input
                  value={addDocForm.displayName}
                  onChange={(e) => setAddDocForm((f) => f && { ...f, displayName: e.target.value })}
                  onKeyDown={(e) => e.key === 'Enter' && addOwnDoc()}
                  placeholder="표시명"
                  className="flex-1 text-[12px] border border-sb-n200 rounded-[6px] px-2 py-1.5 bg-white focus:outline-none focus:border-sb-brand"
                />
              </div>
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={addDocForm.isRequired}
                    onChange={(e) =>
                      setAddDocForm((f) => f && { ...f, isRequired: e.target.checked })
                    }
                    className="rounded"
                  />
                  <span className="text-[12px] text-sb-n600">필수</span>
                ...
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={addDocForm.isConditional}
                    onChange={(e) =>
                      setAddDocForm((f) => f && { ...f, isConditional: e.target.checked })
                    }
                    className="rounded"
                  />
                ...
                <button
                  onClick={addOwnDoc}
                  disabled={!addDocForm.type.trim() || !addDocForm.displayName.trim()}
                  className="flex items-center gap-1 px-3 py-1.5 rounded-[6px] text-[12px] font-medium bg-sb-brand text-white disabled:opacity-40"
                >
                  <Plus size={12} /> 추가
                </button>
                <button
                  onClick={() => setAddDocForm(null)}
                  className="text-[12px] text-sb-n500 hover:text-sb-n800"
                >
                  취소
                </button>
              </div>
...
function ruleToRow(rule: EntityClassificationRule): EntityRow {
  const btCond = rule.conditions.find(
    (c: EntityClassificationCondition) => c.field === 'businessType'
  )
  const fcCond = rule.conditions.find(
    (c: EntityClassificationCondition) => c.field === 'foundingCountry'
  )
...
function rowToRule(row: EntityRow, priority: number): EntityClassificationRule {
  ...
  if (row.businessType !== 'any')
    conditions.push({ field: 'businessType', op: 'eq', value: row.businessType })
  if (row.foundingCountry === 'KR')
    conditions.push({ field: 'foundingCountry', op: 'eq', value: 'KR' })
  else if (row.foundingCountry === 'overseas')
    conditions.push({ field: 'foundingCountry', op: 'neq', value: 'KR' })
  const btLabel = BT_OPTIONS.find((o) => o.value === row.businessType)?.label ?? row.businessType
  const fcLabel =
    FC_OPTIONS.find((o) => o.value === row.foundingCountry)?.label ?? row.foundingCountry
  return {
    id: row.id,
...
function EntityClassificationTableEditor({ code }: { code: EntityCode }) {
  ...
  function addRow() {
    saveRows([
      ...myRows,
      { id: `ecr_${Date.now()}`, businessType: 'any', foundingCountry: 'any', result: code },
    ])
  }
  ...
  const sel =
    'w-full text-[12px] border border-sb-n200 rounded-[6px] px-2 py-1.5 bg-white focus:outline-none focus:border-sb-brand text-sb-n700'

  return (
    <div className="flex flex-col gap-4">
      <p className="text-[12px] text-sb-n400">
        이 세그먼트로 분류되는 조건 행을 편집합니다. 결과 세그먼트를 변경하면 해당 행은 다른
        세그먼트의 뷰로 이동합니다.
      </p>
      ...
      <div className="bg-white rounded-[10px] border border-sb-n100 overflow-hidden">
        ...
        {myRows.map((row, idx) => (
          <div
            key={row.id}
            className="grid grid-cols-[1fr_1fr_1fr_36px] gap-2 px-4 py-2.5 border-b border-sb-n100 last:border-0 items-center"
          >
            <select
              value={row.businessType}
              onChange={(e) => updateRow(idx, { businessType: e.target.value as RowBusinessType })}
              className={sel}
            >
              {BT_OPTIONS.map((o) => (
                <option key={o.value} value={o.value}>
            ...
            <select
              value={row.foundingCountry}
              onChange={(e) =>
                updateRow(idx, { foundingCountry: e.target.value as RowFoundingCountry })
              }
              className={sel}
            >
              {FC_OPTIONS.map((o) => (
            ...
            <select
              value={row.result}
              onChange={(e) => updateRow(idx, { result: e.target.value as EntityCode })}
              className={sel}
            >
...
function ServiceConditionEditor({ code }: { code: ServiceCode }) {
  ...
  const registeredCountries = Array.from(
    new Set(
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white rounded-[10px] border border-sb-n100 p-5">
            ...
            <p className="text-[13px] text-sb-n500">
              수금 선택 + 미등록 국가인 경우 이 세그먼트로 자동 분류됩니다.
            </p>
            <p className="text-[11px] text-sb-n400 mt-1 font-mono">
              collection + country not in any registered Collection segment
            </p>
          </div>
          <p className="text-[11px] text-sb-n400">
            이 조건은 코드로 관리되며 패널에서 편집할 수 없습니다.
          </p>
        </div>
      )
    }
    return (
      <p className="text-[13px] text-sb-n400">
        이 서비스 코드에 분류 규칙이 없습니다. (SVC_PAYOUT는 2차 인테이크에서 수동 지정)
      </p>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="text-[12px] text-sb-n400">
        1차 인테이크 폼에서 어떤 값이 선택될 때 이 서비스가 트리거되는지 설정합니다.
      </p>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-1.5 min-h-[24px]">
            {rule.triggerCountries.map((c) => (
              <span
                key={c}
                className="flex items-center gap-1 px-2.5 py-1 rounded-full bg-sb-blue-100 text-sb-brand text-[12px] font-mono font-medium"
              >
                {c}
                <button
                  onClick={() =>
                    updateRule({ triggerCountries: rule.triggerCountries.filter((x) => x !== c) })
                  }
            ...
            {rule.triggerCountries.length === 0 && (
              <p className="text-[12px] text-sb-n400">추가된 국가 없음</p>
            )}
          </div>
        </div>
      </div>
...
function AddCountryWizard({
  ...
  function validateStep1() {
    ...
    if (!d.serviceCode || d.serviceCode === 'SVC_')
      errs.serviceCode = '코드를 입력하세요 (예: SVC_IDR)'
  ...
  function validateStep2() {
    const errs: Record<string, string> = {}
    if (d.triggerServices.length === 0)
      errs.triggerServices = '최소 하나의 서비스 조건이 필요합니다'
    setErrors(errs)
    return Object.keys(errs).length === 0
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-[16px] shadow-xl w-full max-w-[560px] mx-4 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-sb-n100">
          <div>
            <p className="text-[11px] text-sb-n400 font-medium uppercase tracking-[0.5px]">
              국가 추가 ({step}/4)
            </p>
          ...
          <div className="flex gap-1.5 items-center">
            {([1, 2, 3, 4] as const).map((s) => (
              <div
                key={s}
                className={`rounded-full transition-all ${s === step ? 'w-5 h-2 bg-sb-brand' : s < step ? 'w-2 h-2 bg-sb-brand/40' : 'w-2 h-2 bg-sb-n200'}`}
              />
            ))}
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            <div className="flex flex-col gap-5">
              <div>
                ...
                <input
                  value={d.serviceCode}
                  onChange={(e) =>
                    setD((p) => ({ ...p, serviceCode: e.target.value.toUpperCase() }))
                  }
                  placeholder="SVC_IDR"
                ...
                {errors.serviceCode ? (
                  ...
                  <p className="text-[12px] text-sb-n400 mt-1">
                    SVC_로 시작하는 고유 코드. 예: SVC_IDR, SVC_CNY, SVC_PHP
                  </p>
                )}
              </div>
              <div>
                ...
                />
                {errors.displayName && (
                  <p className="text-[12px] text-sb-negative mt-1">{errors.displayName}</p>
                )}
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="flex flex-col gap-5">
              <div>
                <label className="block text-[13px] font-semibold text-sb-n700 mb-1.5">
                  서비스 선택 조건
                </label>
                <p className="text-[12px] text-sb-n400 mb-3">
                  1차 인테이크에서 어떤 서비스가 선택될 때 이 세그먼트가 트리거되는지 설정합니다.
                </p>
                ...
                {errors.triggerServices && (
                  <p className="text-[12px] text-sb-negative mt-1">{errors.triggerServices}</p>
                )}
              </div>
              ...
              <div>
                ...
                <p className="text-[12px] text-sb-n400 mb-3">
                  이 세그먼트를 트리거할 국가 코드를 추가합니다. 비어 있으면 국가 무관.
                </p>
                    ...
                    >
                      ...
                      <button
                        onClick={() =>
                          setD((p) => ({
                            ...p,
                            triggerCountries: p.triggerCountries.filter((x) => x !== c),
                          }))
                        }
                        className="ml-0.5 hover:text-sb-negative leading-none"
                      >
          ...
          {step === 3 && (
            <div className="flex flex-col gap-4">
              <p className="text-[13px] text-sb-n600">
                이 세그먼트에서 고객에게 표시할 공통 질문을 선택합니다. 세그먼트 고유 질문은 완료 후
                Questions 탭에서 추가할 수 있습니다.
              </p>
              <div className="rounded-[10px] border border-sb-n100 overflow-hidden">
                {commonQuestions.map((q, i) => (
                  <label
                    key={q.id}
                    className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-sb-n50 ${i > 0 ? 'border-t border-sb-n100' : ''}`}
                  >
                    <input
                      type="checkbox"
                    ...
                    <span className="text-[10px] font-mono text-sb-n400 px-1.5 py-0.5 border border-sb-n100 rounded bg-white flex-shrink-0">
                      {q.inputType}
                    </span>
                  </label>
                ))}
          ...
          {step === 4 && (
            <div className="flex flex-col gap-4">
              <p className="text-[13px] text-sb-n600">
                기본 서류 목록을 입력합니다. 완료 후 Documents 탭에서 언제든 수정할 수 있습니다.
              </p>
              <div className="rounded-[10px] border border-sb-n100 p-4">
                {d.baseDocs.length === 0 && (
                  <p className="text-[12px] text-sb-n400 py-1">추가된 서류 없음</p>
                )}
                <div className="flex flex-col divide-y divide-sb-n100 mb-1">
                  {d.baseDocs.map((doc, i) => (
                    <div key={doc.type} className="flex items-center gap-3 py-2">
                      ...
                      <span
                        className={`text-[10px] rounded-full px-2 py-0.5 flex-shrink-0 ${doc.isRequired ? 'bg-red-50 text-sb-negative' : 'bg-sb-n50 text-sb-n500'}`}
                      >
                      ...
                      <button
                        onClick={() =>
                          setD((p) => ({
                            ...p,
                            baseDocs: p.baseDocs.filter((_, idx) => idx !== i),
                          }))
                        }
                ...
                <AddDocForm
                  onAdd={(doc) => setD((p) => ({ ...p, baseDocs: [...p.baseDocs, doc] }))}
                />
              </div>
            </div>
          )}
  ...
  function handleWizardFinish(data: WizardData) {
    ...
    updateRuleSet({
      ...
      serviceClassificationRules: [
        ...
        {
          serviceCode: newCode,
          triggerServices: data.triggerServices,
          triggerCountries: data.triggerCountries,
        },
      ],
      segmentQuestionConfigs: [
        ...
        {
          key: `service:${newCode}`,
          enabledCommonQuestionIds: data.enabledCommonQuestionIds,
          ownQuestions: data.ownQuestions,
        },
      ],
      documentRules: [
    ...
    <div className="min-h-screen bg-sb-n50">
        ...
        <div className="max-w-[1200px] mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[13px] text-sb-n400 hover:text-sb-negative transition-colors"
            >
              <SignOut size={15} />
              로그아웃
            </button>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-6 flex gap-6">
        ...
        <aside className="w-[220px] flex-shrink-0 flex flex-col gap-1">
          {/* 인테이크 group */}
          <p className="text-[10px] font-semibold text-sb-n400 uppercase tracking-[0.8px] px-3 pt-2 pb-1">
            인테이크
          </p>
          <button
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isIntake ? 'bg-sb-brand' : 'bg-sb-n300'}`}
            />
          ...
          {/* Entity group */}
          <p className="text-[10px] font-semibold text-sb-n400 uppercase tracking-[0.8px] px-3 pt-3 pb-1">
            Entity
          </p>
          {ENTITY_ORDER.map((code) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${isActive ? 'bg-sb-brand' : 'bg-sb-n300'}`}
                />
                <span className="text-[13px] leading-snug">{rs.entityLabels[code]}</span>
                <span className="ml-auto text-[10px] font-mono text-sb-n400">
                  {code.replace('ENTITY_', 'E·')}
                </span>
              </button>
            )
          })}
            ...
            <div>
              <div className="flex items-center gap-3 pb-4 border-b border-sb-n100 mb-0">
                <div>
                  ...
                  <h2 className="text-[18px] font-semibold text-sb-n900">
                    {rs.entityLabels[selected.code]}
                  </h2>
                </div>
                <span className="ml-auto text-[11px] font-mono bg-sb-n50 border border-sb-n100 px-2 py-1 rounded text-sb-n500">
                  {rs.version}
              ...
              {entityTab === 'classification' && (
                <EntityClassificationTableEditor code={selected.code} />
              )}
            ...
            <div>
              <div className="flex items-center gap-3 pb-4 border-b border-sb-n100 mb-0">
                <div>
                  <p className="text-[11px] font-mono text-sb-n400">{selected.code}</p>
                  <h2 className="text-[18px] font-semibold text-sb-n900">
                    {rs.serviceLabels[selected.code]}
                  </h2>
                </div>
                <span className="ml-auto text-[11px] font-mono bg-sb-n50 border border-sb-n100 px-2 py-1 rounded text-sb-n500">
                  {rs.version}
