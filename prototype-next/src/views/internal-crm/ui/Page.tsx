...
import { useSalesActionStore } from '@/src/entities/case/model/salesActionStore'
import { STATUS_LABELS } from '@/src/features/case-state/model/stateMachine'
import type { CaseStatus } from '@/src/shared/type'
    ...
    return (
      ...
      >
        <div className="text-center">
          <p className="text-[14px] mb-3" style={{ color: 'var(--sb-n500)' }}>
            CRM은 영업 담당자만 접근할 수 있습니다.
          </p>
          <button
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--sb-n50)' }}>
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        <div className="mb-6">
          <h2 className="text-[18px] font-bold" style={{ color: 'var(--sb-n900)' }}>
            종료 케이스 관리
          </h2>
          <p className="text-[13px] mt-1" style={{ color: 'var(--sb-n500)' }}>
            종료된 케이스에 후속 조치를 기록하고 관리합니다.
          </p>
        </div>
        ...
        {closedCases.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {closedCases.map((c) => {
              ...
              const prevStatus = lastHistory.find(
                (h) => h.previousStatus && h.newStatus === 'CLOSED'
              )
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    <span className="text-[13px]" style={{ color: 'var(--sb-n600)' }}>
                    ...
                    <span
                      ...
                      style={
                        c.closeReason === 'DROPPED' ? { color: 'var(--sb-negative)' } : undefined
                      }
                    >
                      {c.closeReason ? CLOSE_REASON_LABEL[c.closeReason] : '—'}
                    </span>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[12px]" style={{ color: 'var(--sb-n600)' }}>
                        {SEGMENT_LABEL[
                          c.segmentInfo?.entity ?? c.segmentInfo?.entitySegment ?? ''
                        ] ??
                    ...
                    >
                      ...
                      {prevStatus?.notes && (
                        <div className="rounded-[8px] p-3" style={{ background: 'var(--sb-n50)' }}>
                          <p className="text-[11px] mb-1" style={{ color: 'var(--sb-n400)' }}>
                            종료 사유
                          </p>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            setActionInputs((prev) => ({ ...prev, [c.id]: e.target.value }))
                          }
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') submitAction(c.id)
                          }}
                        />
                        <button
