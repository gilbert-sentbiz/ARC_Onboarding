...
import {
  classify,
  classifySectors,
} from '@/src/features/case-classification/model/segmentClassifier'
import {
  canTransition,
  STATUS_LABELS,
  ROLE_LABELS,
} from '@/src/features/case-state/model/stateMachine'
import { buildDocuments } from '@/src/features/document-requirements/model/documentRequirements'
import type {
  Case,
  CaseStatus,
  IntakeRecord,
...
export function saveFirstIntakeDraft(
  ...
  if (existing && existing.firstIntake.status === 'draft') {
    const segmentInfo = classify({
      ...
      collectionCountries: Array.isArray(formData.collectionCountries)
        ? formData.collectionCountries
        : [],
...
function resolveOwner(newStatus: CaseStatus): { role: UserRole; name: string } {
  ...
  if (targetRole) {
    ...
    if (staff.length > 0) {
      ...
      const counts = staff.map(
        (s) =>
          cases.filter((c) => c.currentOwner.role === targetRole && c.currentOwner.name === s.name)
            .length
      )
