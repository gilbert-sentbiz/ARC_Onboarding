...
export default function Input({ label, required, error, helper, iconLeft, iconRight, className = '', ...rest }: Props) {
  ...
  return (
    <div className={`flex flex-col gap-1.5 ${className}`}>
      ...
      >
        {iconLeft && (
          <span className="flex-shrink-0" style={{ color: 'var(--sb-n400)' }}>{iconLeft}</span>
        )}
        <input
        ...
        />
        {iconRight && (
          <span className="flex-shrink-0" style={{ color: 'var(--sb-n400)' }}>{iconRight}</span>
        )}
      </div>
      {error && (
        <p className="text-[11px] leading-[16px] tracking-[0.055px]" style={{ color: 'var(--sb-negative)' }}>{error}</p>
      )}
      {!error && helper && (
        <p className="text-[11px] leading-[16px] tracking-[0.055px]" style={{ color: 'var(--sb-n500)' }}>{helper}</p>
      )}
    </div>
  )
}
