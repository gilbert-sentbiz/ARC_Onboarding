-- ============================================================
-- V2__seed_rules.sql — MVP 룰 시드
-- 세그먼트(CORP/INDIV/SVC_PAYOUT) · 질문(1차 14 + 2차 65) · 서류 템플릿(9) · 직원(4)
-- UUID 체계:
--   a0000001-... ENTITY_CORP / a0000002-... ENTITY_INDIV / a0000003-... SVC_PAYOUT
--   c*  1차 질문 / d*  2차 공통 / e*  2차 CORP 고유 / f*  2차 INDIV 고유
--   g*  doc_template / h*  staff
-- ============================================================
...
--    고객이 가입 후 최초로 작성하는 설문. 분류 판정의 입력값.
-- ──────────────────────────────
...
-- ──────────────────────────────
-- 4. question — 2차 공통 (phase='second', classification='common')
--    ENTITY_CORP · ENTITY_INDIV 모두 표시. 5문항 + VASP 꼬리 5
-- ──────────────────────────────
INSERT INTO question (id, code, phase, classification, label, input_type, is_required, options, show_when, display_order) VALUES
...
-- VASP 꼬리 — show_when qc_virtual_asset=yes
INSERT INTO question (id, code, phase, classification, label, input_type, is_required, options, show_when, parent_question_id, display_order) VALUES
  ...
  ('d0000009-0001-0000-0000-000000000000', 'qc_vasp_purpose', 'second', 'common',
   'SentBe 이용 목적 (가상자산 관련)',
   'textarea', true, null,
   '{"question_id":"d0000004-0001-0000-0000-000000000000","value":"yes"}',
   'd0000004-0001-0000-0000-000000000000', 5);

-- VASP 해외 라이선스 꼬리 — show_when qc_vasp_outside_lic=yes
INSERT INTO question (id, code, phase, classification, label, input_type, is_required, options, show_when, parent_question_id, display_order) VALUES
...
-- ──────────────────────────────
-- 5. question — 2차 CORP 고유 (phase='second', classification='own', owner=ENTITY_CORP)
-- ──────────────────────────────
...
INSERT INTO question (id, code, phase, classification, owner_segment_id, label, input_type, is_required, options, parent_question_id, display_order) VALUES
  ...
  ('e0000015-0001-0000-0000-000000000000', 'qe_corp_rep_gender',  'second', 'own', 'a0000001-0001-0000-0000-000000000000', '성별', 'radio', true,
   '[{"value":"male","label":"남"},{"value":"female","label":"여"}]',
   'e0000011-0001-0000-0000-000000000000', 4),
...
INSERT INTO question (id, code, phase, classification, owner_segment_id, label, input_type, is_required, options, display_order) VALUES
  ('e0000017-0001-0000-0000-000000000000', 'qe_corp_bo_exempt', 'second', 'own', 'a0000001-0001-0000-0000-000000000000',
   '25% 이상 지분 보유 실소유자 면제 사유',
   'radio', true,
   '[{"value":"none","label":"해당 없음 (실소유자 입력 필요)"},{"value":"listed","label":"국내 상장법인"},{"value":"government","label":"국가·지방자치단체"},{"value":"financial","label":"금융기관"}]',
   11);

-- BO 조건부 질문 (show_when qe_corp_bo_exempt=none)
INSERT INTO question (id, code, phase, classification, owner_segment_id, label, input_type, is_required, options, show_when, display_order) VALUES
  ('e0000018-0001-0000-0000-000000000000', 'qe_corp_bo_has_25', 'second', 'own', 'a0000001-0001-0000-0000-000000000000',
   '25% 이상 지분 보유자 존재 여부',
   'radio', true,
   '[{"value":"yes","label":"예"},{"value":"no","label":"아니요"}]',
   '{"question_id":"e0000017-0001-0000-0000-000000000000","value":"none"}', 12),
  ('e0000019-0001-0000-0000-000000000000', 'qe_corp_bo_count', 'second', 'own', 'a0000001-0001-0000-0000-000000000000',
   '25% 이상 지분 보유자 수',
   'number', true, null,
   '{"question_id":"e0000017-0001-0000-0000-000000000000","value":"none"}', 13);

-- BO 반복 그룹 (show_when qe_corp_bo_exempt=none)
INSERT INTO question (id, code, phase, classification, owner_segment_id, label, input_type, is_required, repeat, options, show_when, display_order) VALUES
  ('e0000020-0001-0000-0000-000000000000', 'qe_corp_bo_group', 'second', 'own', 'a0000001-0001-0000-0000-000000000000',
   '실소유자 정보', 'text', true, true, null,
   '{"question_id":"e0000017-0001-0000-0000-000000000000","value":"none"}', 14);
...
INSERT INTO question (id, code, phase, classification, owner_segment_id, label, input_type, is_required, options, display_order) VALUES
  ('e0000026-0001-0000-0000-000000000000', 'qe_corp_purpose', 'second', 'own', 'a0000001-0001-0000-0000-000000000000',
   ...
   '[{"value":"trade","label":"무역 결제"},{"value":"payroll","label":"급여 송금"},{"value":"investment","label":"투자"},{"value":"other","label":"기타"}]',
   15);
...
-- ──────────────────────────────
-- 6. question — 2차 INDIV 고유 (phase='second', classification='own', owner=ENTITY_INDIV)
-- ──────────────────────────────
...
-- 실소유자 별도 입력 (show_when qe_indiv_bo_same=no)
INSERT INTO question (id, code, phase, classification, owner_segment_id, label, input_type, is_required, options, show_when, parent_question_id, display_order) VALUES
