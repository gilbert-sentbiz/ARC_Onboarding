...
## 2026-08-27 — 계약서 전 세그먼트 항상 선택 (PI-256)

> Gilbert 결정. 계약서(`CONTRACT`)를 모든 세그먼트에서 optional로.

- CORP 서류 **필수 8→7, 계약서를 선택으로**(INDIV는 기존에도 선택). `doc_template`의 `CONTRACT` `is_required=false`.
- PRD §4.4 서류표·서류 조건 갱신(한/영). 티켓 PI-256.

## 2026-08-27 — 고객 제출 응답 재열람 추가 (PI-248)

> Gilbert 결정. 고객이 제출 후 자기 1·2차 답을 다시 볼 화면이 없던 갭 보완.

- **상태 & 이력 화면에 "제출한 답변 보기"(읽기 전용)** 추가 — 1·2차 질문+답 + 세그먼트 판정 결과. 조회 API C7(`GET /cases/{id}/intake/{phase}`) 연동, 수정 불가, 불변 질문 계보 기준 매칭.
- PRD §2.1 고객 화면 + 상태 & 이력 설명 갱신(한/영). 티켓 PI-248.
