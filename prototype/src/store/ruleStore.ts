...
function mergeSegQuestionConfigs(
  stored: import('../types').SegmentQuestionConfig[] | undefined,
  initial: import('../types').SegmentQuestionConfig[]
): import('../types').SegmentQuestionConfig[] {
  if (!stored?.length) return initial
  const initialByKey = new Map(initial.map(c => [c.key, c]))
  const storedKeys = new Set(stored.map(c => c.key))
  const merged: import('../types').SegmentQuestionConfig[] = stored.map(c => ({
    ...c,
    commonOptionFilters: c.commonOptionFilters ?? initialByKey.get(c.key)?.commonOptionFilters,
  }))
